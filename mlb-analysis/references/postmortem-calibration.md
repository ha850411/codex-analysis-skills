...
先套用 `../../shared/postmortem-improvement.md`；只調低 confidence component、注碼或推薦資格不得作為 postmortem PR 的唯一修正。
...
- 修改 confidence 計算不會改變勝率或得分分布，必須與預測模型修正分開評估；若 PR 只有 confidence 變更，判定為風控調整而非精準度改善並拒絕建立 postmortem PR。
...
依序說明：可否取得正式原快照、賽果與實際事件、批次評分、可預見錯誤、合理變異、可否證修正假設、基準版／挑戰版比較與是否通過升版條件。若只有單場資料，結論只能是「流程錯誤已由回歸測試確認」或「校準警報待累積」，不能宣稱模型整體已改善。
