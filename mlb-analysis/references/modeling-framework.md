...
本文件適用於所有新 MLB 機率。目標是預測「機率分布是否校準」，不是猜中單一精確比分。若無法把證據轉成可重現的得分率或情境權重，只能把它列為風險或信心度缺口，不得憑敘事手調勝率。
...
「不得給強結論」表示降低信心度、擴大情境範圍或停止推薦，不表示自動取消整份預測。已有版本化基準模型時，使用可追溯的 pre-lineup／未知環境情境產生基準分布；只有連基準 run means 都無法重現時才把數值保留為 `N/A`。
...
- 使用 `scripts/simulate_scores.py` 時，輸入的各階段期望得分、共同環境波動與隊伍波動參數必須來自回測，不得臨場猜值。腳本只負責把已建立的 run means 轉成一致的市場機率。
...
- 未有可重現 run means、分布參數或同版本回測：只給方向與敏感區間，不得輸出假精準勝率或注碼。
- 合理情境的差異足以翻轉推薦或明顯改變 EV 時，對投注使用較保守端，不用點估計的完整優勢。

上述閘門只控制數值結論與投注推薦，不控制報告交付。排程確認有賽事但零場可建模時，仍須輸出 `degraded` 報告，逐場保存 `status`、`missing_data`、`sources`、資料截止時間與 N/A 欄位，供資料可用率稽核與賽後流程檢討；不得把這類紀錄納入 Brier、log loss 或得分誤差。
