...
4. 目前的 Codex 只讀 `model-input.json`，依 `references/prediction.schema.json` 完成主預測並寫入 `primary_prediction.json`。不要另開一個 Codex CLI。
5. 執行以下命令，讓 agy-cli 產生 `red_team_review.json`：

   ```bash
   python3 prediction-pipeline/scripts/pipeline.py red-team \
     --run-dir <執行目錄> --agy-agent "<已確認的紅隊模型>"
   ```
...
- 主預測只能看到 `model_data`；在機率與信心度固定前，必須從檔案層級隔離 `market_data`。
