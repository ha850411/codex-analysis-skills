...
本文件只在實際執行 agy 或批次管線時載入。CLI 範例從 repo 根目錄執行；設定值以當次 model-defaults 和使用者指示為準。新 canonical forecast 先使用共用 pipeline-input 轉接，computed_probability_groups 存在時不得由任何模型改寫。
...
在分析前編輯 [model-defaults.json](model-defaults.json)，即可預選整條管線的模型：
...
1. 讀取並驗證 `model-defaults.json`；若無效則停止並指出欄位，不得靜默改用其他模型。
...
2. 完成即時資料收集，依 `input.schema.json` 建立執行目錄：
...
5. 執行以下命令，讓 agy-cli 產生 `red_team_review.json`：
   ...
   明確傳入已告知的 `--agy-model`，並用 `--domain-skill` 傳入本次使用的領域 `SKILL.md`；管線會先以 `agy models` 重驗模型，再用 `agy --model` 呼叫，並把該技能及其 `output-template.md` 放進可信審查契約，使 `domain_report_coverage` 真正按各運動模板檢查。`--agy-agent` 僅保留為舊指令的相容別名，不得再表示 agy agent。若省略 model，程式會讀取設定檔預選值並在呼叫前告知。
6. 依已告知的最終裁決模型執行：
   - 選擇「目前會話模型」：由目前的 Codex 讀取完整輸入、主預測與紅隊報告，依 `final.schema.json` 寫入 `final_prediction.json`。
...
7. 若 `market_data` 非空，先建立確定性市場快照：
   ...
   目前 Codex 只在此時讀取 `market_comparison.json`，依 `post-market.schema.json` 寫入 `post_market_decision.json`。這個 artifact 必須逐一裁決全部 `bet_id`、揭露市場玩法覆蓋完整度，並回填簡表每一列的「投注建議」；不得包含或修改任何機率。若只取得獨贏而未取得使用者要求或該領域通常可玩的讓分、總局數、精確比分、首局等市場，標記 `market_coverage.status=partial` 並列出未取得玩法。
...
- 裁決後保留 primary 的章節標題作覆蓋識別，逐項回答適用問題；不適用者保留原因。可以刪除重複文字，不再用字數比率衡量完整性。輸出依 `../../shared/forecast/report-template.md`，另產生 `chat-summary.md`。
...
建立收集器、手動修復 JSON 或擴充欄位時，閱讀 [資料契約](contracts.md) 與 `` 內的 JSON Schema。
