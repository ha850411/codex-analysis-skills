...
3. **近期狀態分析**
   ...
   - 每場預測 artifact 必須列出雙方同一賽事最近至少兩個系列（不足則列全部）的日期、對手、比分、版本與實際五人。只寫「公開資料不足」而未保存逐隊查找結果，不得用跨 Split 強度先驗直接取代這批證據。
...
6. **預測模型**
   ...
   - 新預測的 evidence artifact 必須讓驗證器重算中央點：至少包含基準先驗、同賽事近期內容與弱方取勝三個模型；有可比直接再戰時，其反模型輸出與權重必須同時出現在 H2H 與中央集成，兩處不一致即停止鎖定機率。
