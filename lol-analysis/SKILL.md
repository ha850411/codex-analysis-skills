...
- 輸出採「結果詳盡、分析精簡」：聊天先列雙方勝率、完整系列比分、適用的至少一局／讓局／總局數機率與價格決策，再用最多三個短論點交代關鍵對位、雙方勝法與 BP 翻轉條件；欄位與 BO3／BO5 格式依 `references/output-template.md`。深入請求增加證據查核與結果解讀，不自動增加過程敘述；查核、重試與模型稽核細節留在附件。
