...
6. **預測模型**
   ...
   - 提供雙方整體勝率。
   - 提供雙方各自至少贏一局的機率，以及「雙方皆至少贏一局」的機率。三項都是每場必要數據，必須在單場卡片與置底簡表中直接顯示；不得只提供精確比分後要求讀者自行加總，也不得藏在 toggle、附註或方法段落。輸出時直接使用兩隊的實際隊名，不得寫「主隊／客隊」或把「隊伍 A／隊伍 B」當成對外標籤；隊伍順序固定沿用賽程／事件順序並與比分分布一致。
...
9. **最終表格**
   ...
   - 每張卡片在精確比分分布後，緊接一個固定四欄的「至少拿一局」區塊：`指標 | {實際隊名 1} | {實際隊名 2} | 雙方皆至少一局`。三個機率必須與主分布一致並明確帶 `%`；欄名不得加上「主隊／客隊」或輸出「隊伍 A／隊伍 B」占位字樣。
   - 置底簡表的「至少一局機率」欄逐場固定列出三行：`{實際隊名 1} {%}`、`{實際隊名 2} {%}`、`雙方 {%}`。不得只列「雙方皆至少一局」，也不得省略任一隊。
