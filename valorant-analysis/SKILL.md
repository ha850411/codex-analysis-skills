...
3. 每個情境列出權重、完整 ban/pick、pick owner、map order、逐圖機率與精確比分分布。情境權重合計 100%。
...
四場以上的批次檢討必須執行 `scripts/audit_batch.mjs`，至少回報勝負 Brier / log loss、精確比分 log loss、預期與實際橫掃場數、比分眾數集中度，以及實際 lineup / veto 是否被賽前情境覆蓋。樣本很小時只能標記診斷，不得宣稱已完成長期校準。
