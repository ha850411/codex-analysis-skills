...
每次引用 Injury Report 都記錄報告日期、發布時間與擷取時間。新版報告、球隊正式公告或可信 beat reporter 的臨場更新出現後，建立 post-lineup／late-news 新快照，不覆寫舊預測。
...
1. Odds-API.io 的 Stake bookmaker 指定快照（`shared/markets/collect_odds_api.mjs --sport nba`）
2. 使用者提供且較新的 Stake 即時賠率
3. 使用者指定的 sportsbook
4. 多市場平均價格作參考

在模型鎖定後擷取快照並保存 provider event ID、Stake 深連結、擷取時間與原始回應 SHA-256。無法擷取或未開盤時才改用使用者／指定來源；只取得 ML 必須標為部分覆蓋，不能暗示讓分、大小分、節次或球員盤已檢查。
...
- 任何傷兵、先發或盤口更新都要保留 `as_of`；盤口變動不回寫模型勝率或信心度。
