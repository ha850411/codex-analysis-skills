...
- 使用者要求檢討失準、回測或改善模型時，先讀 `references/postmortem-calibration.md`，重建原預測快照後再評估。
...
以下是起始配置，不是固定常數；依 `prediction-methodology.md` 對小樣本收縮並避免巢狀時間窗重複計權：

- 近 14 天或同賽事：40%
- 第 15–30 天：20%
- 更長期同陣容、同地圖池基準：25%
- 對手強度、LAN/online、賽程與角色脈絡：15%
...
依 `../shared/analysis-core.md` 選擇輸出模式。`full`、`daily-summary` 與 `postmortem` 讀 `references/output-template.md`；`quick` 只保留結論、核心地圖／veto 證據、主要風險與資料狀態。
