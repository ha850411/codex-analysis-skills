...
9. 不得修改 shared、其他 skill、automation、Git 設定或原始預測。不得自行 commit、push 或建立 PR；外層程式會處理。
10. 若修改 skill，採「最小充分修改」：修正應完整涵蓋已證實的錯誤機制，不以行數小為目標，也不無限追加賽後特例。既有輸出章節、表格、欄位、順序與 JSON key 不得新增、刪除、改名或重排，也不得修改 `mlb-analysis/references/output-template.md`。在 postmortem.md 記錄 baseline/challenger、測試、回退方式與仍未解問題。
