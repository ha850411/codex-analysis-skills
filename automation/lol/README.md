...
`predict_next_day.py` 使用 bo3.gg 公開 API 預查從 `automation/modules.json` 的 `lol.schedule.prediction` 起算、起點含且終點不含的 24 小時視窗。只有該視窗至少包含一場等級恰好為 `s` 的 LoL 賽事時，才會啟動 Codex。通過驗證的產物會發布為一筆 Notion `daily-summary`，接著寄送 SMTP Email。
