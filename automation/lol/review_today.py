...
4. 檢討以未來樣本的勝方命中優先、精確比分命中次之；同時檢查 Brier、log loss 與覆蓋率。降低信心度、注碼、推薦等級或語氣只是附帶風控，不得作為 skill 修正或 PR 的唯一內容。
...
def main() -> int:
    ...
    try:
        with job_lock("review"):
            ...
            paths = changed_paths(worktree)
            run([sys.executable, str(REPO_ROOT / "shared/forecast/bridge.py"), "--sport", "lol",
