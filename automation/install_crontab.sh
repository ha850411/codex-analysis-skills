...

{
  ...
  print -r -- "0 15 * * * cd '${REPO_ROOT}' && CODEX_BIN='${CODEX_BIN}' '${PYTHON_BIN}' automation/run_scheduled.py review --module mlb >> '${LOG_DIR}/mlb-review.log' 2>&1"
  ...
  print -r -- "30 22 * * * cd '${REPO_ROOT}' && CODEX_BIN='${CODEX_BIN}' '${PYTHON_BIN}' automation/run_scheduled.py review --module lol >> '${LOG_DIR}/lol-review.log' 2>&1"
  print -r -- "${END_MARKER}"
} >>"${new_file}"

crontab "${new_file}"
print "自動化排程已設定：MLB 每天 21:00 預測、15:00 檢討；LoL 每天 09:00 預測、22:30 檢討。"
